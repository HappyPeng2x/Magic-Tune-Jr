...
import {durationMs, DEFAULT_SPEED} from './TimelineDuration';
...
export default class Sprite {
    ...
    createSprite (page, md5, id, attr, fcn) {
        ...
        this.timeline = [];  // populated from attr.timeline when loading a saved project
        setProps(this, attr);
    ...
    activate () {
        // Wired to TimelinePane in a later step.
    }
    ...
    // Add a clip at `startTime` ms on the given sub-row `track`.
    // `duration` is calculated from block type and arg if not supplied.
    // Returns the new clip object.
    addClip (blocktype, arg, startTime, track = 0) {
        var speed = this.speed || DEFAULT_SPEED;
        var calculated = durationMs(blocktype, arg, speed);
        var clip = {
            id: this.id + '_clip_' + Date.now() + '_' + Math.floor(Math.random() * 1000),
            blocktype: blocktype,
            arg: arg,
            startTime: startTime,
            duration: calculated !== null ? calculated : 2000,
            track: track
        };
    ...
    removeClip (clipId) {
        this.timeline = this.timeline.filter(function (c) {
            return c.id !== clipId;
        });
    }

    // Merge `changes` into the clip with the given id. Returns the clip or null.
    updateClip (clipId, changes) {
        var clip = this.timeline.find(function (c) {
            return c.id === clipId;
        });
        if (clip) {
