...
export default class UI {
    ...
    static BottomSection () {
        // Scratchjr internals (Page.setCurrentSprite, Thumbs) look for #scripts.
        // In timeline mode there is no scripts pane, but we need the element to
        // exist so those calls don't throw.  Keep it hidden.
        var scripts = newHTML('div', 'scripts', frame);
        scripts.setAttribute('id', 'scripts');
        scripts.style.display = 'none';

        TimelinePane.create(frame);
    }
    ...
    static setShowGrid (b) {
        if (!ScratchJr.stage) return;
        Grid.hide(!b);
        if (gn('grid')) gn('grid').className = Grid.hidden ? 'gridToggle off' : 'gridToggle on';
    }
